...
class Argentum(QtGui.QMainWindow):    
    ...
    def initialiseComms(self, port):
        ...
        try:
            ...
            self.appendOutput('Error connecting on port [{}]: {}'.format(port, sys.exc_info()[0]))
            return False

    def monitor(self):
        if(self.serial.inWaiting()):
            self.appendOutput(self.serial.readline())
    ...
    def homeButtonPushed(self):
        self.sendCommand('M X 0\n')
        self.sendCommand('M Y 0\n')
    ...
    def sendMovementCommand(self, x, y):
        if x:
            self.sendCommand('M X {}\n'.format(x))
            
        if y:
            self.sendCommand('M Y {}\n'.format(y))
